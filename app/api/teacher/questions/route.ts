...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await getAdminClient().from("questions").insert({ upload_id: body.uploadId || null, author_id: user.id, subject: values[5], prompt: values[0], option_a: values[1], option_b: values[2], option_c: values[3], option_d: values[4], correct_option: body.correctOption, explanation: typeof body.explanation === "string" ? body.explanation.trim() || null : null, status: "draft" }).select().single();
...
export async function PATCH(request: Request) {
  try {
    ...
    const { data, error } = await getAdminClient().from("questions").update({ status: "published", published_at: new Date().toISOString() }).eq("id", id).eq("author_id", user.id).select("id,status").single();
    if (error) throw error;
